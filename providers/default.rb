...
# Copyright:: Copyright (c) 2017-2026 Progress Software Corporation and/or its subsidiaries or affiliates. All Rights Reserved.
#
...
action :update do
  begin
    ...
    # Display chef-ice (Chef 19+) warning
    if new_resource.product_name == 'chef-ice'
      raise(
        <<~ERROR
          =====================================================================================================
          Unsupported product: chef-ice

          This cookbook does not support upgrading to Chef Infra Client 19+ (chef-ice).
          The chef_client_updater cookbook only supports omnibus versions of Chef Infra Client 18 and below.

          For Chef Infra Client 19+, please use alternative installation methods.
          =====================================================================================================
        ERROR
      )
    end
    ...
    if update_necessary?
      converge_by "upgrade #{new_resource.product_name} #{current_version} to #{desired_version}" do
        ...
        Chef::Log.debug("Install script generated by mixlib-install:\n#{install_script}\n")
